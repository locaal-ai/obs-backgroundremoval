...
MaskEditorDialog::MaskEditorDialog(QWidget *parent, const cv::Mat& mask8UC1) : QDialog(parent), isDrawing(false) {
  ...
  obs_log(LOG_INFO, "MaskEditorDialog: mask8UC1.cols = %d, mask8UC1.rows = %d, mask8UC1.step = %d", mask8UC1.cols, mask8UC1.rows, mask8UC1.step);
  // Convert the mask image to a QImage
  QImage maskQImage = QImage(mask8UC1.data, mask8UC1.cols, mask8UC1.rows, mask8UC1.step,
  ...
  this->painter->setPen(QPen(Qt::white, 20, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
...
bool MaskEditorDialog::eventFilter(QObject *obj, QEvent *event) {
  ...
  if (event->type() == QEvent::MouseMove && this->isDrawing) {
    ...
    obs_log(LOG_INFO, "Mouse move event: x = %d, y = %d", mousePos.x(), mousePos.y());

    // draw a white circle on the mask image at the mouse position
    // this->painter->drawEllipse(mousePos, 10, 10);

    // update the mask image
    maskImage->setPixmap(QPixmap::fromImage(maskImage->pixmap().toImage()));
