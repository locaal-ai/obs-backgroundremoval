	...
	: QDialog(parent), layout(new QVBoxLayout)
{
  ...
  scrollArea->setWidget(label);
	...
	// Add a checkbox
	segmentTracingEnableCheckbox = new QCheckBox("Collect usage statistics");
  bool shouldEnable;
  if (getFlagFromConfig("segment_tracing", &shouldEnable) == OBS_BGREMOVAL_CONFIG_SUCCESS) {
    if (shouldEnable) {
	...
	connect(segmentTracingEnableCheckbox, &QCheckBox::stateChanged, this,
		[](int state) { setFlagFromConfig("segment_tracing", state == Qt::Checked); });

  startupCheckbox = new QCheckBox("Show this dialog on startup");
  startupCheckbox->setCheckState(Qt::Unchecked);
  connect(startupCheckbox, &QCheckBox::stateChanged, this,
    [](int state) {
      setFlagFromConfig("show_segment_tracing_dialog", state == Qt::Checked);
    });
	...
	connect(closeButton, &QPushButton::clicked, this, &SegmentTracingDialog::closeDialog);
}

void SegmentTracingDialog::closeDialog()
{
  // Save config value for showing the dialog on startup
  setFlagFromConfig("show_segment_tracing_dialog", startupCheckbox->checkState() == Qt::Checked);
  // Save config value for segment tracing
  setFlagFromConfig("segment_tracing", segmentTracingEnableCheckbox->checkState() == Qt::Checked);
