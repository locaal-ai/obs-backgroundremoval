...
int send_segment_trace(int segment_type, int info) {
  ...
  obs_data_set_obj(traceData, "metadata", metadata);
    ...
    return SEND_SEGMENT_TRACE_SUCCESS;
		...
		obs_log(LOG_WARNING, "Error while putting trace segments: %s %s",
...
void segment_tracing_init(void) {
  bool showSegmentTracingDialog;
	if (getFlagFromConfig(
		"show_segment_tracing_dialog", &showSegmentTracingDialog) == OBS_BGREMOVAL_CONFIG_FAIL) {
...
void segment_tracing_init_tracing(void) {
  ...
  bool shouldEnable;
  if (getFlagFromConfig("segment_tracing", &shouldEnable) == OBS_BGREMOVAL_CONFIG_FAIL) {
    obs_log(LOG_WARNING, "Failed to read segment tracing config, cannot enable segment tracing");
    return;
  }
  if (!shouldEnable) {
  ...
  if (send_segment_trace(SEGMENT_TYPE_PLUGIN_INIT_TRACE, 0) != SEND_SEGMENT_TRACE_SUCCESS) {
    obs_log(LOG_WARNING, "Failed to send plugin init trace. Segment tracing will be disabled.");
    delete xrayClient;
    xrayClient = nullptr;
    return;
  }
...
void segment_tracing_show_dialog(void) {
  dialog = new SegmentTracingDialog((QWidget *)obs_frontend_get_main_window());
